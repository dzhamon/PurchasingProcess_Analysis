...
def advanced_procurement_analysis(df):
    ...
    if MATCHER_AVAILABLE:
        ...
        try:
            comparable_groups = find_comparable_products(
                converted_df,
                threshold=0.85,
                method='combined',
                verbose=True
            ...
            print(f"[DEBUG] Результат: найдено {len(comparable_groups)} групп")

            if comparable_groups:
                print(f"\n✓ Найдено {len(comparable_groups)} групп сопоставимых товаров")

                # Выявление критических расхождений
                critical_groups = [g for g in comparable_groups if g['price_diff_pct'] > 30]

                print(f"[DEBUG] Из них критических (>30%): {len(critical_groups)}")

                if critical_groups:
                    print(f"\n⚠️ КРИТИЧЕСКИЕ РАСХОЖДЕНИЯ В ЦЕНАХ: {len(critical_groups)} групп")
                    print("-" * 80)

                    # Детальный отчет
                    for i, group_data in enumerate(critical_groups[:5], 1):  # Показываем топ-5
                        print(f"\n┌─ Группа {i} {'─' * 67}┐")
                        print(f"│ Поставщиков: {group_data['count']}")
                        print(f"│ Разница цен: {group_data['price_diff_pct']:.1f}%")
                        print(f"│ Диапазон: {group_data['min_price']:,.0f} - {group_data['max_price']:,.0f} UZS")
                        print("├" + "─" * 78 + "┤")
                        ...
                        for item in group_data['group']:
                            ...
                            print(f"│ Цена: {item['price']:>15,.0f} UZS  (+{price_vs_min:>5.1f}%) {marker}")
    ...
    for product_type in converted_df['product_type'].unique():
        ...
        if len(subset) >= 2 and subset['counterparty_name'].nunique() >= 2:
            price_by_supplier = subset.groupby('counterparty_name')['unit_price_uzs'].agg(['mean', 'count'])
...
# ================= БЛОК 3. АНАЛИЗ КОНКУРЕНТНОСТИ ============
    ...
    for discipline in df['discipline'].unique():
        ...
        top_supplier = discipline_df.groupby('counterparty_name')['total_amount_uzs'].sum().sort_values(ascending=False)
