...
class AnalyzeActorsEfficients:
    def __init__(self, df):
        ...
        self.OUT_DIR = os.path.join(BASE_DIR, 'Supplier_Frequency')
    ...
    def analyze_supplier_frequency(self):
        ...
        for project in set_projects:
            ...
            project_dir = os.path.join(self.OUT_DIR, f"{project}")
            ...
            for discipline, group_discipline in grouped_data:
                ...
                excel_path = os.path.join(
                    discipline_dir, f"{discipline.replace(' ', '_')}_supplier_frequency.xlsx"
                )
    ...
    def analyze_supplier_behavior(df):
        """
        :param df: DataFrame с информацией о Лота, который формируется на этапе загрузки данных
        :return:
        """
        output_dir = "D:/Analysis-Results/Supplier-Behavior"
        ...
        file_path = os.path.join(output_dir, "bidding_duration_analysis.xlsx")
    
        # Группируем данные, чтобы получить среднюю продолжительность для каждой пары (actor, winner)
        grouped_duration = (
            df.groupby(["discipline", "actor_name", "winner_name"])["duration_days"]
            .agg(["count", "mean"])
            .reset_index()
        )
        ...
        discipline_avg = df.groupby("discipline")["duration_days"].mean().reset_index()
        ...
        # Рассчитываем разницу в продолжительности
