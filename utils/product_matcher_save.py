...
def fast_find_comparable_products(df, threshold=0.85, parallel_threshold=10000):
    ...
    print("\n2. Поиск совпадений внутри категорий...")

    # 🔥 ЗАЩИТА ОТ ПЕРЕПОЛНЕНИЯ ПАМЯТИ
    MAX_GROUP_SIZE = 500  # Максимум товаров в группе
    ...
    for key, group in df.groupby('key'):
        ...
        # 🔥 ПРОВЕРКА: Пропускаем группы с category=None
        if category == 'None':
        ...
        # 🔥 ПРОВЕРКА: Ограничиваем размер группы
        if group_size > MAX_GROUP_SIZE:
            print(f"   ⚠️ БОЛЬШАЯ ГРУППА: {category} / {product_type}: {group_size} товаров")
            print(f"      Обработка только топ-{MAX_GROUP_SIZE} по цене...")

            # Берём топ-N самых дорогих (там больше вероятность найти переплаты)
            group = group.nlargest(MAX_GROUP_SIZE, 'unit_price_eur')
            group_size = MAX_GROUP_SIZE

        print(f"   Обработка: {category} / {product_type}: {group_size} товаров")
    ...
    # Итоговая статистика
    print(f"\n✅ Найдено совпадений: {len(all_matches)}")

    if skipped_groups > 0:
        print(f"\n⚠️ ПРЕДУПРЕЖДЕНИЕ:")
        print(f"   Пропущено групп: {skipped_groups}")
        print(f"   Пропущено товаров: {skipped_items}")
        print(f"   Причина: category=None или группа >500 товаров")
